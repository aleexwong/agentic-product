...
```mermaid
flowchart TB
    A["🎤 Microphone Audio"] --> B["Web Speech API"]
    B --> C["Browser-Local Text Processing"]
    C --> D["Buzzword Detection\n(Client-side only)"]
    D --> E["Game State Update\n(localStorage)"]

    note1["✅ Audio never leaves device\n✅ No server transmission\n✅ No recording or storage\n✅ Transcription discarded after processing"]
```
...
graph TD
    ...
    CategorySelection --> CategoryCard["CategoryCard ×3"]
    CategorySelection --> BackButton

    GameBoard --> GameHeader
    GameBoard --> BingoCard
    GameBoard --> TranscriptPanel
    GameBoard --> GameControls

    GameHeader --> Logo
    GameHeader --> StatusIndicator["StatusIndicator\nlistening/paused"]
    GameHeader --> ProgressCounter

    BingoCard --> BingoSquare["BingoSquare ×25"]

    TranscriptPanel --> LiveTranscript
    TranscriptPanel --> DetectedWords

    GameControls --> NewCardButton
    GameControls --> ListeningToggle

    WinScreen --> Confetti
    WinScreen --> WinningCard
    WinScreen --> GameStats
    WinScreen --> ShareButton
